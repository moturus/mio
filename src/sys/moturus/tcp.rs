...
pub(crate) fn bind(addr: SocketAddr) -> io::Result<i32> {
    let fd =
        moto_rt::net::bind(moto_rt::net::PROTO_TCP, &addr.into()).map_err(map_moturus_error)?;
    moto_rt::net::set_nonblocking(fd, true).map_err(map_moturus_error)?;
...
pub(crate) fn listen(listener: &net::TcpListener, max_backlog: u32) -> io::Result<()> {
    ...
    moto_rt::net::listen(fd, max_backlog).map_err(map_moturus_error)
}
